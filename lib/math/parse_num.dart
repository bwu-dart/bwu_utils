...
bool isDouble(dynamic s, {
  ...
  num parsed = parseNum(s,
      ...
      acceptNegativeInfinity:
        acceptNegativeInfinity);
  return parsed != null &&
      (parsed.toString().contains('.') || parsed.toString().contains('e')) ||
...
double parseDouble(dynamic s, {double onErrorDefault: null,
  ...
  if(!acceptNegativeInfinity && result == double.NEGATIVE_INFINITY) return onErrorDefault;
...
class ParseResult<T> {
  ...
  bool operator==(dynamic other) {
    ...
    return other.value == value && other.unit == unit && other.isError == isError;
  }
...
ParseResult<double> parseDoubleWithUnit(String s) {
  ...
  Match m = new RegExp(r'(?:\s*)(-?[0-9]*\.?[0-9]*)(?:\s*)([^\s]*)').firstMatch(s);
